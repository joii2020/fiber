...
use fiber_types::{
    ...
    AwaitingTxSignaturesFlags, ChannelState, CollaboratingFundingTxFlags, HashAlgorithm,
    InMemorySigner, NegotiatingFundingFlags, OutboundTlcStatus, PaymentHopData, PaymentStatus,
    Privkey, RemoveTlcFulfill, RemoveTlcReason, ShuttingDownFlags, SigningCommitmentFlags, TLCId,
...
async fn test_reestablish_restores_send_nonce() {
    ...
    // Persist a broken state where `send` is lost but `verify`/`next` remain.
    // This is the actual recovery path handled by reestablish.
    node_b.stop().await;
    let mut persisted_state = node_b.get_channel_actor_state(channel_id);
    assert!(
        persisted_state.remote_revocation_nonce_for_verify.is_some()
            || persisted_state.remote_revocation_nonce_for_next.is_some(),
        "Expected a stored revocation nonce to recover from"
    );
    persisted_state.remote_revocation_nonce_for_send = None;
    persisted_state.last_revoke_ack_msg = None;
    node_b.store.insert_channel_actor_state(persisted_state);

    // Restart node B to simulate reconnecting from the broken persisted state.
    node_b.start().await;
